...
/* ─────────────────────────────────────────────────────────────
 * HomeScreenWithTest (NO-FETCH 버전)
 *  - 테스트에서 전역 stores에 저장된 추천 결과만 읽어서 렌더
 *  - 네트워크 호출 X
 * ────────────────────────────────────────────────────────────*/
export const HomeScreenWithTest = () => {
    ...
    // 상세 오버레이에 전달할 상태/액션
    const isPlanned = activeStore ? plannedVisitIds.includes(activeStore.id) : false;
    const isReviewed = activeStore ? reviewedStoreIds.includes(activeStore.id) : false;
    ...
    const handleWriteReview = () => {
        if (!activeStore) return;
        // 리뷰 작성 화면이 있다면 여기서 navigate(`/review/new/${activeStore.id}`)
        setActiveStoreId(null);
    };
