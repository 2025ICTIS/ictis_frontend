...
export const useAppStore = create<AppState>((set, get) => ({
    ...
    addTestAnswer: (answer) =>
        set((s) => ({testAnswers: [...s.testAnswers, answer]})),
    ...
    markReviewed: (storeId) => {
        ...
        set((s) => ({
            reviewedStoreIds: already
                ? s.reviewedStoreIds
                : [...s.reviewedStoreIds, storeId],
            user: s.user
                ? {
                    ...s.user,
                    reviews: already ? s.user.reviews : (s.user.reviews ?? 0) + 1,
                }
                : s.user,
        }));
    },
    ...
    setUserDemographics: ({gender, ageRange, district}) =>
        set((s) => ({
            user: s.user ? {...s.user, gender, ageRange, district} : null,
        })),
}));
